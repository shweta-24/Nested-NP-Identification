...
def read_test_data(dataset_file_path):
...
test_dataset = read_test_data(dataset_file_path)
...
metrics_file = 'Data/metrics_detailed_OntoNotes.pkl'
...
for ex_num in range(start_idx, len(sentence_ids_test_list)):
    ...
    try:
        ...
        infered_parse = inference_BERT.infer_sentence_parse(list(ex_parse['tokens'])[0])
...
for test_case_num in list(metrics_all):
    sent_len = len(metrics_all[test_case_num]['sentence'].split())
    #sent_len = metrics_all[test_case_num]['tree_depth']        # Uncomment to plot graph along Maximal NP Depth
...
plt.xlabel('Sentence Length')   #Update if plotting across maximal NP depth
plt.ylabel('Recall')
plt.title('Recall vs Sentence Length')  #Update if plotting across maximal NP depth
plt.legend(loc='upper center', bbox_to_anchor=(0.5, 1.05), ncol=3)
fig = plt.gcf()
tikzplotlib_fix_ncols(fig)
plt.savefig('Results/Eng/RecallvsSenLen.png')
tikzplotlib.save("Results/Eng/RecallSenLen.tex")
plt.show()
...
plt.title('Precision vs Sentence Length')  #Update if plotting across maximal NP depth
plt.legend(loc='upper center', bbox_to_anchor=(0.5, 1.05), ncol=3, fancybox=True)
fig = plt.gcf()
tikzplotlib_fix_ncols(fig)
plt.savefig('Results/Eng/PrecisionvsSenLen.png')
tikzplotlib.save("Results/Eng/PrecisionSenLen.tex")
plt.show()
